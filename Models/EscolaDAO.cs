    ...
    internal class EscolaDAO
    {
        ...
        public void Update(Escola escola)
        {
            try
            {
                ...
                comando.CommandText = "UPDATE escola SET nome_fantasia_esc = @nome, razao_social_esc = @razao, cnpj_esc = @cnpj, insc_estadual_esc = @inscricao, tipo_esc = @tipo, data_criacao_esc = @data_cricao, responsavel_esc = @resp, responsavel_telefone_esc = @resp_tel, email_esc = @email, telefone_esc = @telefone, rua_esc = @rua, numero_esc = @numero, bairro_esc = @bairro, complemento_esc = @complemento, cep_esc = @cep, cidade_esc = @cidade, estado_esc = @estado WHERE id_esc = @id";
